...
class RUSHBserver():
    ...
    def compute_checksum(self, message):
        print("Calculate checksum:", message)
        b_str = message
        if len(b_str) % 2 == 1:
            b_str += b'\0'
        checksum = 0
    ...
    def creat_user(self, addr, filecontent, sequence, acknowlege, checksum):
        ...
        print("USER ", addr, self.client[addr]['sequenceNum'], self.client[addr]['ackNum'], self.client[addr]['checksum'],
              "created with", self.client[addr]['file'])
    ...
    def user_settime(self, current_time, addr):
        ...
        print("Time set for user:", addr, "at:", self.client[addr]['time'])
    ...
    def run(self):
        ...
        while True:
            try:
                ...
                # print("User:", addr, "time:", self.user_gettime(addr))
            ...
            if flag == '0010000' and checksum == 0:  # GET
                ...
                print("GET AND CHK")
                print("Cauculated checksum:", self.compute_checksum(massage.encode('ascii')))
                ...
                print(current_client_sequence)
                print(self.user_getrequestorder(addr))
